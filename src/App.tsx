...
const App = () => {
  ...
  const [searchWord, setSearchWord] = useState<string>("bunny");
  ...
  return (
    <div className="mt-6 max-w-[800px] flex flex-col mx-5 md:mx-auto">
      ...
      <form onSubmit={handleSearch} className="mb-4">
        <div
          className={`relative rounded-xl bg-neutral-100 border-2 focus:outline-none transition-colors ${
            isEmptyInput
              ? "border-red-500 focus:border-red-500 hover:border-red-500"
              : "border-neutral-200/50 focus:border-green-500 hover:border-green-500"
          }`}
        >
          <input
            type="text"
            ...
            className={`w-full px-4 py-4 pr-12 text-lg italic font-semibold focus:outline-none`}
          />
          <button
            type="submit"
            className="absolute right-2 top-1/2 transform -translate-y-1/2 hover:text-green-500 px-3 font-medium cursor-pointer"
          >
      ...
      {data && data[0] && !isLoading && !isError && (
        <>
          <div className="flex gap-2 items-center my-3">
            <h1 className="text-4xl sm:text-6xl font-black wrap-anywhere">
              {data[0].word}
            </h1>
            <div className="flex items-end gap-3">
              {data[0].phonetics[0] && data[0].phonetics[0].audio != "" && (
                <>
                  ...
                  <button
                    onClick={playAudio}
                    className="size-7.5 mt-2 sm:mt-3.5 bg-green-400 hover:bg-green-600 text-white rounded-full cursor-pointer transition-colors flex items-center justify-center"
                  >
          ...
          <footer className="mt-6 pt-4 border-t border-gray-200 text-center text-sm text-gray-500">
            ...
            <a
              ...
              target="_blank"
              className="text-green-500 hover:text-green-600 font-medium pb-4"
            >
              @coelhomarcus
            </a>
          </footer>
